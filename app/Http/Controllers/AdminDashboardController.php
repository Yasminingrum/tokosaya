...
class AdminDashboardController extends Controller
{
    ...
    public function index()
    {
        try {
            ...
            return view('admin.dashboard', $this->getDefaultDashboardData());
        }
    }
    ...
    private function getDashboardStats()
    {
        ...
        try {
            if (Schema::hasTable('orders')) {
                ...
                // Today's revenue (in cents, convert to rupiah)
                $todayRevenue = DB::table('orders')
                    ->where('created_at', '>=', $today)
                    ->where('payment_status', 'paid')
                    ->sum('total_cents');
                $stats['today_revenue'] = $todayRevenue / 100;
                ...
                if ($yesterdayRevenue > 0) {
                    $stats['revenue_trend'] = round((($stats['today_revenue'] - $yesterdayRevenue) / $yesterdayRevenue) * 100, 1);
                }

                // Today's orders
                $stats['today_orders'] = DB::table('orders')
                    ->where('created_at', '>=', $today)
                    ->count();
                ...
                if ($yesterdayOrders > 0) {
                    $stats['orders_trend'] = round((($stats['today_orders'] - $yesterdayOrders) / $yesterdayOrders) * 100, 1);
                }
            ...
            if (Schema::hasTable('users')) {
                ...
                $stats['new_customers'] = DB::table('users')
